                             ...
                             verbose = TRUE, plots = FALSE, tol = 1e-6, max.it = 100, criterion="bic")
{
  ...
  if(classif == "hard"){
    ...
    for (l in 1:nclust) {
      if(is.null(r) == TRUE){
        spenv.var[[l]] = getEnvVar(sp.xy=sp_iter[[l]], env.grid=env.xy, coord = c("X", "Y"), 
                                 env.scale = sp.scale)
        
        ppmprep[[l]] = ppmdat(sp.xy=sp_iter[[l]], back.xy = env.xy, sp.scale = sp.scale,
                              coord = c("X", "Y"), sp.dat = spenv.var[[l]])
    ...
    while(abs(loglik.new - loglik.old)/abs(loglik.old) > tol) {
      ...
      if(is.null(cov.bias)){
        unkcov.mat = getEnvVar(unknown.xy, env.xy, env.scale = sp.scale, coord = c("X", "Y"))
        ...
        unkpred.mat = getEnvVar(unknown.xy, pred.env.xy, env.scale = sp.scale, coord = c("X", "Y"))
        predint = matrix(NA, sum(unknown), nclust)
        for (m in 1:nclust) {
      ...
      for (i in 1:nclust) {
        if(is.null(r) == TRUE){
          spenv.var[[i]] = getEnvVar(sp.xy=sp_iter_op[[i]], env.grid=env.xy, coord = c("X", "Y"), 
                                   env.scale = sp.scale)
          
          ppmprep[[i]] = ppmdat(sp.xy=sp_iter_op[[i]], back.xy = env.xy, sp.scale = sp.scale,
                                coord = c("X", "Y"), sp.dat = spenv.var[[i]])
  ...
  if(classif == "soft"){
    ...
    for (l in 1:nclust) {
      if(is.null(r) == TRUE){
        spenv.var[[l]] = getEnvVar(sp.xy=ppp.df[[l]], env.grid=env.xy, coord = c("X", "Y"), 
                                 env.scale = sp.scale)
                              ...
                              verbose = TRUE, plots = FALSE, tol = 1e-6, max.it = 100, criterion="bic")
{
  ...
  for (l in 1:n.sp) {
    if(is.null(r) == TRUE){
      spenv.var[[l]] = getEnvVar(sp.xy=sp.xy[[l]], env.grid=env.xy, coord = c("X", "Y"), 
                               env.scale = sp.scale)
  ...
  while(breakloop == 0)
  {
    ...
    for (l in 1:n.sp) {
      if(is.null(r) == TRUE){
        spenv.var[[l]] = getEnvVar(sp.xy=sp_aug[[l]], env.grid=env.xy, coord = c("X", "Y"), 
                                 env.scale = sp.scale)
